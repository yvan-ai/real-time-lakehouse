...
if command -v terraform >/dev/null 2>&1; then
  info "terraform found — applying infra/terraform/local (Strimzi + Flink operator)."
  terraform -chdir="${REPO_ROOT}/infra/terraform/local" init -input=false >/dev/null
  terraform -chdir="${REPO_ROOT}/infra/terraform/local" apply -input=false -auto-approve
else
  warn "terraform not found — falling back to the operator install scripts."
